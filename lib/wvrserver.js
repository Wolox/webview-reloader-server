...
var indexHtmlSource = fs.readFileSync(require.resolve('./index.html'), 'utf-8');
...
function watchListenerFolder(listener) {
  ...
  fs.watch(watchedFolder, function (event, filename) {
    ...
    if (listener.fileRegexp && !listener.fileRegexp.test(filename)) {
...
function rootRequestHandler(req, res) {
  console.log("Serving client index.html with parameters", req.query);
  res.setHeader('Content-Type', 'text/html');
  res.writeHead(200);
  res.end(indexHtmlSource);
}
